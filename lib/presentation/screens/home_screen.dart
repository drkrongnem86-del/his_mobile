...
// v3.0.177: Refresh user picker cache (2000 users) trong "Làm mới dữ liệu"
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /// v3.0.177: "Làm mới dữ liệu" - trước chỉ setState(() {}) no-op
  /// Giờ chạy 3 task SONG SONG (parallel) để tận dụng latency:
  ///   1) Reload DS BN khoa hiện tại (qua _departmentKey._load())
  ///   2) Refresh khoa từ HIS Pro API (DepartmentService.refresh)
  ///   3) Refresh user picker cache (UserService.load forceRefresh)
  /// Sau đó snackbar feedback thành công/thất bại
  Future<void> _onRefreshData() async {
    context.safePop();
    if (!mounted) return;
    _snack('Đang làm mới dữ liệu...', color: Colors.blue);

    final results = await Future.wait<bool>([
      // 1. Reload BN khoa hiện tại (_load() đã có try/catch)
      Future<bool>(() async {
        try {
          await _departmentKey.currentState?._load();
          return true;
        } catch (_) {
          return false;
        }
      }),
      // 2. Refresh khoa từ HIS Pro API (false nếu chưa login)
      DepartmentService.instance.refresh(),
      // 3. Refresh user picker (2000 users, cần login HIS Pro)
      Future<bool>(() async {
        try {
          await UserService.instance.load(forceRefresh: true);
          return UserService.instance.isLoaded;
        } catch (_) {
          return false;
        }
      }),
    ...
    if (!mounted) return;
    setState(() {}); // rebuild UI cho cả _departments getter

    final okBn = results[0];
    final okDept = results[1];
    final okUser = results[2];
    if (okBn && (okDept || okUser)) {
      _snack(
        '✅ Đã làm mới BN${okDept ? " + khoa" : ""}${okUser ? " + user picker" : ""}',
        color: Colors.green,
      );
    } else if (okBn) {
      _snack('✅ Đã làm mới BN (khoa + user cần login HIS Pro)', color: Colors.orange);
    } else {
      _snack('❌ Lỗi làm mới dữ liệu', color: Colors.red);
    }
  }
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  // v3.0.177: Fix "Làm mới dữ liệu" - trước chỉ setState(() {}) no-op
                  // Giờ: reload DS BN khoa hiện tại + refresh khoa + user cache
                  //         + snackbar feedback (đang tải / xong / lỗi)
                  _drawerItem(
                    icon: Icons.refresh,
                    color: Colors.blue,
                    title: 'Làm mới dữ liệu',
                    subtitle: 'Tải lại DS BN + khoa + user picker',
                    onTap: _onRefreshData,
                  ),
