...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    // v2.48.0: Tự động bootstrap HIS Pro silent
    WidgetsBinding.instance.addPostFrameCallback((_) {
      HisProApiService.instance.silentBootstrap();
    });
  }
  ...
  /// v2.37.1: Tính gợi ý BN từ _patients hiện tại + PatientSeed
  void _updateSuggestions(String q) {
    ...
    // Lấy BN hiện tại trong khoa + seed để mở rộng
    final deptKey = _departments[_currentDeptIndex]['code'] as String?;
    final all = <Map<String, dynamic>>[];
    if (deptKey != null) all.addAll(PatientSeed.getByDepartment(deptKey));
    // Bổ sung các khoa khác
    for (final d in ['HSCC', 'CCS', 'DTYC', 'DVNTK', 'DVTMCT', 'HSTC', 'NTK', 'NTTN', 'KM', 'NGCT', 'NGTH', 'KN', 'NTM', 'KNTH', 'KPS', 'KTNT', 'PTGMHS', 'KRHM', 'KTMH', 'KTN', 'KYHCT', 'KKB', 'KUB', 'KSNK', 'HSCCL', 'CCSVL', 'DTCCLuu']) {
      if (d != deptKey) all.addAll(PatientSeed.getByDepartment(d));
    }
  ...
  Widget _buildHisProSyncButton() {
    final hasToken = HisProService.instance.isLoggedIn;
    return SizedBox(
      width: 110,
      height: 60,
      child: ElevatedButton.icon(
        onPressed: _syncFromHisPro,
        style: ElevatedButton.styleFrom(
          backgroundColor: hasToken ? const Color(0xFFE65100) : Colors.white,
          foregroundColor: hasToken ? Colors.white : const Color(0xFFE65100),
  ...
  Future<void> _syncFromHisPro() async {
    try {
      // Step 1: Login HIS Pro (nếu chưa login)
      if (!HisProService.instance.isLoggedIn) {
        final logged = await _showHisProLoginDialog();
        if (logged != true) return;
      }

      // Step 2: Fetch patients by department
      ...
      if (deptId == null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Khoa này chưa cấu hình ID')),
        );
        return;
      }

      // Show progress dialog với step-by-step feedback
      // v2.38.6: nhỏ gọn + không chiếm full screen + step text update
      final progressLabel = ValueNotifier<String>('Đang kết nối HIS Pro...');
      if (!mounted) return;
      showDialog(
        context: context,
      ...
      progressLabel.value = 'Bước 1/2: Đang tải BN từ HIS Pro...';
      final result = await HisProService.instance.fetchPatientsByDepartment(deptId)
          .timeout(const Duration(seconds: 12), onTimeout: () => HisProSyncResult(
            success: false,
            message: 'Timeout 12s - HIS Pro không phản hồi. Kiểm tra VPN BV.',
            patients: const [],
          ));
      if (!mounted) return;
      ...
      if (!result.success) {
        ...
        // Nếu lỗi token → reset session
        if (result.httpStatus == 401) {
          HisProService.instance.clearSession();
        }
        return;
      }

      // Forward data tới DepartmentPatientPageState để update _patients ở đó
      if (mounted) {
        _departmentKey.currentState?._updateFromHisPro(
          result.patients,
          dept['name']?.toString() ?? '',
        );
      }
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('✅ Đã đồng bộ ${result.patients.length} BN từ HIS Pro'),
      ...
      debugPrint('_syncFromHisPro exception: $e\n$st');
      if (!mounted) return;
      try {
        Navigator.of(context, rootNavigator: true).pop();
      } catch (_) {}
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('❌ Lỗi đồng bộ: ${e.toString().split("\n").first}'),
        backgroundColor: Colors.red.shade700,
        duration: const Duration(seconds: 5),
      ));
    }
  ...
  Future<bool?> _showHisProLoginDialog() async {
    // v2.75.3: Auto-fill từ SharedPreferences nếu đã lưu
    final prefs = await SharedPreferences.getInstance();
    final savedLogin = prefs.getString('hispro_saved_login') ?? 'nemk';
    final savedPass = prefs.getString('hispro_saved_pass') ?? '';
    var rememberPass = savedPass.isNotEmpty;
    final loginCtrl = TextEditingController(text: savedLogin);
    final passCtrl = TextEditingController(text: savedPass);
    ...
    return showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        ...
        title: const Row(
          children: [
            Icon(Icons.cloud_sync, color: Color(0xFFE65100)),
            SizedBox(width: 8),
            Text('Đăng nhập HIS Pro'),
          ],
        ),
        content: Form(
          key: formKey,
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.vpn_lock, size: 16, color: Colors.amber),
                    const SizedBox(width: 6),
                    Expanded(
                      child: Text(
                        rememberPass
                            ? 'Đã lưu mật khẩu (bấm Đăng nhập để sync)'
                            : 'Cần bật OpenVPN BV trước khi đồng bộ',
              ...
              TextFormField(
                ...
                validator: (v) => (v == null || v.trim().isEmpty) ? 'Nhập tài khoản' : null,
              ),
              const SizedBox(height: 10),
              TextFormField(
                controller: passCtrl,
                obscureText: !rememberPass, // v2.75.3: show nếu đã lưu
                decoration: InputDecoration(
                  ...
                  suffixIcon: IconButton(
                    icon: Icon(rememberPass ? Icons.visibility : Icons.visibility_off, size: 18),
                    tooltip: rememberPass ? 'Ẩn mật khẩu' : 'Hiện mật khẩu',
                    onPressed: () {
                      // v2.75.3: Nếu đã lưu thì toggle show/hide
              ...
              const SizedBox(height: 4),
              // v2.75.3: Checkbox "Lưu mật khẩu"
              Row(
                children: [
                  Checkbox(
                    value: rememberPass,
                    onChanged: (v) {
                      setSt(() => rememberPass = v ?? false);
                    },
                  ),
                  const Text('Lưu mật khẩu cho lần sau', style: TextStyle(fontSize: 12)),
                ],
              ),
              const SizedBox(height: 4),
              Text(
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            onPressed: () async {
              ...
              // v2.75.3: Lưu mật khẩu nếu user tick
              final p = prefs;
              if (rememberPass) {
                await p.setString('hispro_saved_login', loginCtrl.text.trim());
                await p.setString('hispro_saved_pass', passCtrl.text);
              } else {
                await p.remove('hispro_saved_pass');
                await p.setString('hispro_saved_login', loginCtrl.text.trim());
              }
              // v2.38.6: KHÔNG Navigator.pop trước khi login xong - để dialog hiển thị "Đang đăng nhập"
              // Thay bằng cách hiển thị overlay loading trên dialog login hiện tại
              showDialog(
                context: ctx,
                barrierDismissible: false,
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 8),
                              Text('Đang đăng nhập HIS Pro...', style: TextStyle(fontSize: 12)),
                            ],
                          ),
                        ),
                      ),
              ...
              final res = await HisProService.instance.login(
                loginCtrl.text.trim(),
                passCtrl.text,
              );
              if (!mounted) return;
              // Đóng TẤT CẢ dialog: overlay loading + login dialog
              Navigator.of(context, rootNavigator: true).pop(); // close overlay
              Navigator.of(context, rootNavigator: true).pop(); // close login dialog
              // Give Flutter a tick to settle before opening sync
              await Future.delayed(const Duration(milliseconds: 100));

              if (res.success) {
                if (!mounted) return;
                await _syncFromHisPro();
              } else {
                if (!mounted) return;
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        ...
        children: [
          ...
          Expanded(
            child: DepartmentPatientPage(
              ...
              onSyncFromHisPro: () => _syncFromHisPro(),
            ),
          ),
        ],
      ),
  ...
  // v3.0.42: Search bar + dòng filter TREATMENT_TYPE_NAME (chips: Tất cả / Nội trú / Khám / Ngoại trú)
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Dòng 2: Filter TREATMENT_TYPE_NAME (v3.0.57 - dynamic từ data HIS Pro)
          // - Trước đó dùng 'Chưa khám / Đang khám' (trạng thái khám) - sai theo BS
          // - Fix: dùng TREATMENT_TYPE_NAME field thật từ HIS Pro (vd: Nội trú / Khám / Ngoại trú)
          // - Dynamic: lấy unique values từ _patients, nếu rỗng thì dùng 3 giá trị mặc định
          SizedBox(
            height: 36,
            child: Row(
              children: [
                ...
                const SizedBox(width: 4),
                const Text('Loại ĐT:', style: TextStyle(fontSize: 11, color: Colors.black54, fontWeight: FontWeight.w600)),
                const SizedBox(width: 6),
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      ...
                      // v3.0.57: Dynamic chips từ TREATMENT_TYPE_NAME
                      ..._getUniqueTreatmentTypeNames().map((t) => _filterChip(t, t)),
                    ],
                  ),
                ),
              ],
  ...
  /// v3.0.42: Chip filter loại điều trị
      ...
      builder: (ctx) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            ...PatientApiSource.values.map((src) {
...
class DepartmentPatientPage extends StatefulWidget {
  ...
  final VoidCallback? onSyncFromHisPro;
...
class _DepartmentPatientPageState extends State<DepartmentPatientPage>
  ...
  // v2.98.2: Default = API 1 (HIS Pro 1408) theo yêu cầu BS
  PatientApiSource _apiSource = PatientApiSource.hisPro;
  String _apiSourceLabel = 'API 1';
  ...
  DateTime _dateFrom = DateTime.now().subtract(const Duration(days: 7)); // v2.49.0: 7 ngày gần (HIS closed = 0 BN nếu chỉ hôm nay)
  DateTime _dateTo = DateTime.now().add(const Duration(days: 1));        // v2.49.0: +1 ngày để bao gồm hôm nay
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (_) => DepartmentPatientPage(
          ...
          onSyncFromHisPro: widget.onSyncFromHisPro,
        ),
      ),
    );
  }
  ...
  void initState() {
    ...
    // v2.77.0: Set PatientSeed NGAY trong setState để render BN frame đầu
    // Nếu gán trực tiếp _patients = seedList, build đầu vẫn dùng _patients=[] mặc định
    final seedList = PatientSeed.getByDepartment(widget.department['code'] as String? ?? 'HSCC');
    setState(() {
      _patients = seedList;
      _dataSource = PatientDataSource.appCode;
      _loaded = true;
      _loading = false;
      _debugInfo = '📦 Dữ liệu mẫu (offline) khoa=${widget.department['code']} → ${seedList.length} BN';
    });
    debugPrint('===== _DepartmentPatientPage initState: seed ${seedList.length} BN for ${widget.department['code']}');
    // Sau đó vẫn gọi _load() để thử lấy BN thật từ API
    WidgetsBinding.instance.addPostFrameCallback((_) => _load());
  }

  // v2.91.0: Load + save _apiSource preference
  // v2.98.2: Default = API 1 (HIS Pro) khi chưa set lần nào
  void _loadApiSourcePreference() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final idx = prefs.getInt('patient_api_source') ?? PatientApiSource.hisPro.index;
  ...
  Future<void> _loadImpl() async {
    ...
    try {
      ...
      // v2.76.8: Nếu CHƯA authenticated → dùng PatientSeed (offline, tên sạch) luôn
      // Tránh app hiện empty ở nhà khi không có VPN
      if (!_data.isAuthenticated) {
        final seedList = PatientSeed.getByDepartment(deptCode ?? 'HSCC');
        debugPrint('===== _loadImpl: not auth → PatientSeed ${seedList.length} BN for $deptCode');
        if (!mounted) return;
        setState(() {
          _patients = seedList;
          _dataSource = PatientDataSource.appCode;
          ...
          _loaded = true;
          _lastSynced = null;
          _debugInfo = '📦 Dữ liệu mẫu (offline) khoa=$deptCode → ${seedList.length} BN. Đăng nhập Data để lấy BN thật.';
        });
        return;
      }
      ...
      if (_data.isAuthenticated && deptId != null && !isPKCapCuu &&
        ...
        try {
          ...
          // v2.76.8: LUÔN fallback PatientSeed (kể cả PK cấp cứu) khi Data không trả BN
          if (!mounted) return;
          final seedList = PatientSeed.getByDepartment(deptCode ?? 'HSCC');
          debugPrint('===== _loadImpl: data-empty → PatientSeed ${seedList.length} BN for $deptCode');
          setState(() {
            _patients = seedList;
            _dataSource = PatientDataSource.appCode;
            _loading = false;
            _loaded = true;
            _debugInfo = '📦 Khoa $deptCode (Data không trả BN) → dùng ${seedList.length} BN mẫu offline. Bật VPN để xem BN thật.';
          });
          return;
        } catch (e) {
          debugPrint('Data service error: $e');
          // v2.76.8: Lỗi Data → fallback PatientSeed (offline)
          final seedList = PatientSeed.getByDepartment(deptCode ?? 'HSCC');
          if (!mounted) return;
          setState(() {
            _patients = seedList;
            ...
            _loading = false;
            _loaded = true;
            _debugInfo = '⚠ Data API lỗi: ${e.toString().split("\n").first} → dùng ${seedList.length} BN mẫu offline.';
          });
          return;
        }
      }
      ...
      // 4. v2.76.8: API không trả về BN (offline / VPN fail / khoa trống) → fallback PatientSeed (offline)
      // Đảm bảo app LUÔN hiển thị tên BN ở mọi khoa
      final seedList = PatientSeed.getByDepartment(deptCode ?? 'HSCC');
      debugPrint('===== _loadImpl: end-of-fn → PatientSeed ${seedList.length} BN for $deptCode');
      if (!mounted) return;
      setState(() {
        _patients = seedList;
        _dataSource = PatientDataSource.appCode;
        _loading = false;
        _loaded = true;
        _debugInfo = '⚠ API $_apiSourceLabel không khả dụng → dùng ${seedList.length} BN mẫu offline. Bật VPN BV để xem BN thật.';
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
  ...
  Widget _buildBody() {
    ...
    if (filtered.isEmpty) {
      // v2.77.0: Bỏ banner cam "PK CẤP CỨU cần OpenVPN" - luôn dùng PatientSeed
      // Chỉ hiện text đơn giản nếu search filter không match
      return Center(
        child: Padding(
          padding: const EdgeInsets.all(20),
          child: Column(
            ...
            children: [
              ...
              Text(
                widget.searchQuery.isNotEmpty
                    ...
                    : 'Chưa có BN trong khoa',
                style: const TextStyle(color: Colors.black, fontSize: 14, fontWeight: FontWeight.w600),
              ),
              const SizedBox(height: 6),
              const Text(
                'Bấm mũi tên ▼ ở trên để chọn khoa',
        ...
        return Container(
          ...
          child: Row(
            children: [
              ...
              IconButton(
                icon: Icon(Icons.refresh, size: 18, color: Colors.deepOrange.shade700),
                onPressed: widget.onSyncFromHisPro,
                tooltip: 'Đồng bộ lại',
                visualDensity: VisualDensity.compact,
                padding: EdgeInsets.zero,
              ),
        ...
        return Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  HisProService.instance.isLoggedIn
                      ? 'Khoa không có BN (Data+HIS Pro đều trống). Bấm "ĐB HIS" để thử lại.'
                      : 'Khoa không có BN. Bật VPN BV → bấm "ĐB HIS" để load PK cấp cứu.',
                  style: const TextStyle(color: Colors.black87, fontSize: 11),
                ),
              ),
              IconButton(
                icon: const Icon(Icons.cloud_sync, size: 18, color: Colors.deepOrange),
                onPressed: widget.onSyncFromHisPro,
                visualDensity: VisualDensity.compact,
                padding: EdgeInsets.zero,
                tooltip: 'ĐB HIS Pro (lấy BN PK cấp cứu)',
              ),
              IconButton(
