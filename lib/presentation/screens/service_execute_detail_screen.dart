...
import 'package:his_mobile/presentation/widgets/kip_thuc_hien_widget.dart';
...
class _ServiceExecuteDetailScreenState extends State<ServiceExecuteDetailScreen> with SingleTickerProviderStateMixin {
  ...
  // v3.0.174: GlobalKey cho KipThucHienForm để lấy kết quả
  ...
  bool _saving = false;
  String _debug = '';
  String? _currentLogin;  // v3.0.174: LOGINNAME user hiện tại (cho KipThucHienForm auto-fill)
  ...
  Future<void> _loadCurrentUser() async {
    try {
      // v3.0.174: Lấy LOGINNAME từ SharedPreferences (key 'pref_hispro_user')
      // thay vì hardcode 'nemk' - cho multi-user
      final prefs = await SharedPreferences.getInstance();
      _currentLogin = prefs.getString('pref_hispro_user') ?? 'nemk';
      debugPrint('ServiceExecute: _currentLogin=$_currentLogin');
    } catch (_) {
      _currentLogin = 'nemk';
    }
  }
  ...
  /// v3.0.174: Dùng KipResult từ KipThucHienForm (thay vì _bsChinhCtrl.text)
  Future<bool> _saveAndFinish() async {
    ...
    // v3.0.174: Lấy kết quả kíp từ KipThucHienForm
    final kipResult = _kipKey.currentState?.result;
    if (kipResult == null || !kipResult.hasPtvChinh) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('⚠️ Vui lòng chọn Bác sĩ / PTV chính (bắt buộc)')),
      );
      return false;
    }
    ...
    try {
      ...
      final formData = {
        ...
        'bs_chinh': kipResult.ptvChinhName,
        'bs_chinh_login': kipResult.ptvChinhLogin,
        'ptv_phu_1': kipResult.ptvPhu1Name,
        'ptv_phu_1_login': kipResult.ptvPhu1Login,
        'ptv_phu_2': kipResult.ptvPhu2Name,
        'ptv_phu_2_login': kipResult.ptvPhu2Login,
        'gay_me_chinh': kipResult.gmChinhName,
        'gay_me_chinh_login': kipResult.gmChinhLogin,
        'gay_me_phu': kipResult.gmPhuName,
        'gay_me_phu_login': kipResult.gmPhuLogin,
        'dd': kipResult.dieuDuongName,
        'dd_login': kipResult.dieuDuongLogin,
      ...
      // v3.0.174: Update ServiceReq với kíp từ KipResult (BEFORE Finish)
      // - Lưu EXECUTE_LOGINNAME/EXECUTER_USERNAME + NURSE_* vào ServiceReq
      // - Lưu PTV/GM vào các fields riêng nếu cần (vd EXECUTE_LOGINNAME2/3/4)
      for (final serviceReqId in _selectedServiceIds) {
        setState(() => _debug = '⏳ Lưu kíp cho serviceReq $serviceReqId...');
        await _api.updateServiceReq({
          'ID': serviceReqId,
          ...kipResult.toServiceReqFields(),
        });
      }
      ...
      for (final serviceReqId in _selectedServiceIds) {
        ...
        // v3.0.173: Chuyển sang Start + Finish (FinishWithTime có thể không tồn tại)
        await _api.startServiceReq(serviceReqId, startTime: _startTime);
        final result = await _api.finishServiceReq(
  ...
  Widget _buildResultTab() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        // Kíp thực hiện (6 vị trí - giống HIS Desktop)
        // v3.0.174: Dùng KipThucHienForm (UserService cache + picker search + auto-fill + remember last)
        _sectionTitle('👥 KÍP THỰC HIỆN (6 vị trí - HIS Desktop)', const Color(0xFFD32F2F)),
        KipThucHienForm(
          key: _kipKey,
