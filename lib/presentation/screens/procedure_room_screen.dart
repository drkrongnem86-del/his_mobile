...
class _ProcedureRoomScreenState extends State<ProcedureRoomScreen> with WidgetsBindingObserver {
  ...
  // v3.0.170: Subscribe TokenEvent để auto-reload khi user paste token mới
  StreamSubscription<TokenEvent>? _tokenSub;
  ...
  void initState() {
    ...
    // v3.0.170: Listen TokenEvent - khi user paste token mới, auto-reload ngay
    _tokenSub = TokenSyncService.instance.listen((event) {
      debugPrint('ProcedureRoom: TokenEvent ${event.type} source=${event.source} → reload');
  ...
  Widget build(BuildContext context) {
    ...
    // v3.0.175: BỎ AppBar "Phòng thử thuật - HSCC" hoàn toàn - chỉ hiện body
    return Scaffold(
      backgroundColor: const Color(0xFFF5F7FA),
      appBar: null,  // Xóa AppBar - tile "Phòng thử thuật - HSCC" không còn ở appbar
      body: body,
    );
  }
