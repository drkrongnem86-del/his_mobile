...
/// v3.0.170: CONVERT to SINGLETON - fix bug HTTP 401 khi TokenSyncService.setToken()
///   tạo instance mới (mỗi screen gọi HisApiService() = new instance riêng, không có token).
///   Tất cả screens (Procedure Room, ECG, Treatment History, Home) phải dùng
///   HisApiService.instance để share cùng Dio + auth headers.
class HisApiService {
  // v3.0.170: Singleton instance
  static final HisApiService instance = HisApiService._init();

  late final Dio _dio;

  /// Private constructor - khởi tạo Dio với headers mặc định HIS Pro
  HisApiService._init() {
  ...
  /// v3.0.170: No-op init() - Dio đã init ở singleton, giữ cho backward-compat
  void init() {}
  ...
  ///
  /// v3.0.173: DEPRECATED - dùng `startServiceReq` + `finishServiceReq` thay thế
  ///   - Endpoint FinishWithTime không tồn tại trên HIS Pro 2.408.0 (BS test fail 24/08)
  ///   - Port sai: ocrUrl (1425) thay vì procedureRoomUrl (1408 MOS)
  /// - Giữ lại cho backward-compat, nội bộ sẽ chuyển sang flow mới
  @Deprecated('Dùng startServiceReq() + finishServiceReq() thay thế')
  Future<HisResult> finishServiceReqWithTime({
    ...
    try {
      // v3.0.173: Chuyển sang dùng Start + Finish (basic) với port 1408 đúng
      // (Endpoint FinishWithTime không tồn tại trên HIS Pro 2.408.0)
      debugPrint('ECG finishServiceReqWithTime (deprecated): chaining Start + Finish');
      // Bước 1: Start (idempotent - nếu status != 1 thì no-op)
      await startServiceReq(serviceReqId);
      // Bước 2: Finish (basic) với time fields
      return await finishServiceReq(
  ...
  /// v3.0.173: Gọi api/HisServiceReq/Start (status 1→2) - port 1408 MOS
  /// Idempotent: nếu status != 1 (đã start hoặc đã finish) thì có thể trả success false nhưng
  /// không sao - flow vẫn tiếp tục đến Finish.
  /// Open source HIS: HIS_DESKTOP/ApiConsumer/HisRequestUriStore.cs:331
  Future<HisResult> startServiceReq(int serviceReqId, {DateTime? startTime}) async {
    try {
      final apiData = <String, dynamic>{
        'ID': serviceReqId,
      };
      if (startTime != null) {
        apiData['START_TIME'] = _fmtHisDateTimeLong(startTime);
      }
      final param = _encodeParam(apiData, 0);
      debugPrint('📡 StartServiceReq (ID=$serviceReqId): $_procedureRoomUrl/api/HisServiceReq/Start');

      final response = await _dio.post(
        '$_procedureRoomUrl/api/HisServiceReq/Start',
        queryParameters: {'param': param},
        options: Options(
          headers: {'Content-Type': 'application/json'},
          receiveTimeout: const Duration(seconds: 30),
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map && data['Success'] == true) {
          debugPrint('✅ StartServiceReq OK: $serviceReqId');
          return HisResult(success: true, data: data['Data']);
        }
        // Idempotent: nếu đã Start/Finish rồi thì success=false cũng OK
        final msg = (data is Map ? data['Message']?.toString() : null) ?? 'Đã start hoặc đã finish';
        debugPrint('ℹ️ StartServiceReq idempotent: $msg');
        return HisResult(success: true, data: data, message: msg);  // success=true vì idempotent
      }
      return HisResult(success: false, message: 'HTTP ${response.statusCode}', data: response.data);
    } catch (e) {
      return HisResult(success: false, message: _handleError(e));
  ...
  /// v3.0.173: Gọi api/HisServiceReq/Finish (status 2→3) - port 1408 MOS
  /// Open source HIS: HIS_DESKTOP/ApiConsumer/HisRequestUriStore.cs:333
  /// Lưu ý: phải Start trước (status 1→2), nếu không sẽ fail
  Future<HisResult> finishServiceReq({
    required int serviceReqId,
    DateTime? startTime,
    DateTime? endTime,
    String resultNote = '',
    ...
    try {
      ...
      if (endTime != null) {
        apiData['END_TIME'] = _fmtHisDateTimeLong(endTime);
        apiData['FINISH_TIME'] = _fmtHisDateTimeLong(endTime);
      }
      if (resultNote.isNotEmpty) {
        apiData['RESULT_NOTE'] = resultNote;
      }
      final param = _encodeParam(apiData, 0);
      debugPrint('📡 FinishServiceReq (ID=$serviceReqId, endTime=$endTime): $_procedureRoomUrl/api/HisServiceReq/Finish');

      final response = await _dio.post(
        '$_procedureRoomUrl/api/HisServiceReq/Finish',
        queryParameters: {'param': param},
        options: Options(
          headers: {'Content-Type': 'application/json'},
          receiveTimeout: const Duration(seconds: 60),
      ...
      if (response.statusCode == 200) {
        final data = _unwrap(response.data);
        if (data is Map && data['Success'] == true) {
          debugPrint('✅ FinishServiceReq OK: $serviceReqId');
          return HisResult(success: true, data: data['Data']);
        }
        return HisResult(
          success: false,
          message: (data is Map ? data['Message']?.toString() : null) ?? 'Finish failed',
          data: data,
        );
      }
      return HisResult(success: false, message: 'HTTP ${response.statusCode}', data: response.data);
  ...
  /// v3.0.173: Format DateTime → yyyyMMddHHmmss long (HIS Pro standard)
  String _fmtHisDateTimeLong(DateTime t) =>
