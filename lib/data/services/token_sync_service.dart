...
// v3.0.169: Bỏ SharedPreferences - token lưu SecureStorage
...
class TokenSyncService {
  ...
  // v3.0.169: SecureStorage key (token nhạy cảm → Android Keystore / iOS Keychain)
  ...
  /// Load token từ SecureStorage + apply cho services
  /// Gọi khi app start (main.dart) hoặc khi mở screen cần token
  Future<void> loadFromStorage() async {
    try {
      // v3.0.169: SecureStorage thay vì SharedPreferences
      final token = await SecureStorageService.instance.read(_kStorageKey);
      if (token != null && token.isNotEmpty) {
        await setToken(token, source: 'cached', eventType: TokenEventType.loaded, broadcast: true);
        debugPrint('✅ [TokenSync] Loaded from SecureStorage: ${token.substring(0, 8)}…');
      } else {
        debugPrint('ℹ️ [TokenSync] No token in SecureStorage');
      }
    } catch (e) {
  ...
  Future<void> setToken(
    ...
    // 1. Lưu vào SecureStorage (v3.0.169: token phải mã hóa)
    try {
      await SecureStorageService.instance.save(_kStorageKey, token);
    } catch (e) {
    ...
    try {
      HisApiService().setAuthToken(token);
    } catch (e) {
  ...
  /// Xóa token (clear SecureStorage + clear services)
  Future<void> clearToken({bool broadcast = true}) async {
    ...
    try {
      // v3.0.169: Xóa khỏi SecureStorage
      await SecureStorageService.instance.delete(_kStorageKey);
    } catch (e) {
      debugPrint('❌ [TokenSync] clear storage error: $e');
    }

    try {
      HisApiService().clearAuthToken();
    } catch (_) {}
    try {
