...
class AutoTokenService {
  ...
  // SharedPreferences keys (chỉ cho metadata, KHÔNG lưu token)
  ...
  // v3.0.169: SecureStorage keys cho token nhạy cảm (Android Keystore / iOS Keychain)
  static const _kSecureToken = 'auto_token_last_token';
  static const _kSecureClientIp = 'auto_token_last_client_ip';
  static const _kStoredPassword = 'auto_token_password';  // đã là SecureStorage từ trước
  ...
  /// Save to SecureStorage (v3.0.169: bảo mật - token phải mã hóa)
  Future<void> _saveCachedToken(AutoTokenResult r) async {
    final prefs = await SharedPreferences.getInstance();
    if (r.token != null) {
      // v3.0.169: Token + IP lưu SecureStorage (Android Keystore / iOS Keychain)
      await SecureStorageService.instance.save(_kSecureToken, r.token!);
      await SecureStorageService.instance.save(_kSecureClientIp, r.clientIp ?? '');
      // Metadata không nhạy cảm → SharedPreferences OK
      await prefs.setString(_kLastSource, r.source.name);
      await prefs.setInt(_kLastFetchTime, DateTime.now().millisecondsSinceEpoch);
    }
  }

  /// Load cached token from SecureStorage (v3.0.169)
  Future<AutoTokenResult?> _loadCachedToken() async {
    final prefs = await SharedPreferences.getInstance();
    // v3.0.169: Token + IP từ SecureStorage
    final token = await SecureStorageService.instance.read(_kSecureToken);
    final ip = await SecureStorageService.instance.read(_kSecureClientIp);
